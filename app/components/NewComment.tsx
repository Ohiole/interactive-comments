...
const NewComment: React.FC<NewCommentProps> = ({ text }) => {
  return (
    <section className='bg-white p-4 rounded-md flex justify-between items-start gap-4 w-[600px] mb-5'>
        ...
        <button className='py-2 px-4 bg-purple cursor-pointer text-sm text-background rounded-md hover:bg-purple-300 transition duration-200'>{text == "reply" ? "REPLY" : "SEND"}</button>
    </section>
  )
}
